...
function doCreateOrUpdate(el, dataIndex, elOption, animatableModel, group, data, isRoot) {
  ...
  if (el && ( // Also consider that if `renderItem` returns nothing, the original element
  // (if exists) will be removed (elOption is an empty object in that case).
  elOptionType == null || elOption.$merge === false || elOptionType !== el.__customGraphicType && (elOptionType !== 'path' || elOption.pathData !== el.__customPathData) && (elOptionType !== 'image' || elOption.style.image !== el.__customImagePath) && (elOptionType !== 'text' || elOption.style.text !== el.__customText))) {
    group.remove(el);
    el = null;
