...
var _roamHelper2 = require("../../action/roamHelper");

var updateCenterAndZoom = _roamHelper2.updateCenterAndZoom;
...
var _default = echarts.extendChartView({
  ...
  render: function (seriesModel, ecModel, api, payload) {
    ...
    if (seriesModel.get('roamAfterExpandAndCollapse') && payload && payload.type === 'treeExpandAndCollapse' && data.getItemGraphicEl(payload.dataIndex)) {
  ...
  _updateViewCoordSys: function (seriesModel) {
    ...
    viewCoordSys.setZoom(zoom); // Here we use viewCoordSys just for computing the 'position' and 'scale' of the group
    ...
    if (!center) {
      var res = updateCenterAndZoom(viewCoordSys, {
        center: center,
        zoom: zoom
      });
...
function symbolNeedsDraw(data, dataIndex) {
  ...
  return layout && !isNaN(layout.x) && !isNaN(layout.y) && data.getItemVisual(dataIndex, 'symbol') !== 'none' && !node.isHide && !node.invisible;
}
