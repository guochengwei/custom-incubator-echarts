...
echarts.registerAction({
  ...
  event: 'treeToggleExpandAndCollapse',
  update: 'update'
...
echarts.registerAction({
  ...
  ecModel.eachComponent({
    ...
    if (dataIndex) {
      ...
      node = node.length === 1 ? node[0] : node.find(function (item) {
    ...
    payload.depth = node.depth; // expandable

    if (node.expandable) {
      node.expandable = false; // replace name
      ...
      try {
        children.forEach(function (item, index) {
          ...
          if (count === 4) {
            item.expandable = true; // replace name
  ...
  ecModel.eachComponent({
    ...
    if (payload.highlight) {
      ...
      tree.root.eachNode(function (node) {
        ...
        node.isExpand = item && item.collapsed != null ? !item.collapsed : node.depth <= initialTreeDepth;
